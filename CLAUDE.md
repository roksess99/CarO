...
| Olie en Filters | `lib/catalog/quick-links.ts`, `components/catalog/actions.ts` | De enige twee uitklapmenu's die er nog zijn, en met reden: hun subgroepen hangen aan de gekozen auto (Citroën C3 heeft er zeven onder `Filter`, McLaren 720S twee) en bestaan niet als pagina. De hoofdgroep staat vast, de subgroepen komen per auto uit een Server Action. Mobiel staan ze als directe link bovenaan het assortimentspaneel |
...
| Meest gezocht | `lib/catalog/quick-links.ts`, `components/catalog/group-list.tsx`, `components/catalog/group-icon.tsx` | Rij met de elf gangbare onderhoudsdelen (inclusief accu's) boven het categorierooster, met het aantal artikelen voor die auto en een eigen lijntekening ervoor. Het zijn **eindgroepen**, dus één klik naar de artikelen. De node-ids zijn op drie auto's gemeten en identiek; of de groep bestáát verschilt wel — niet elke auto heeft een distributieriem. Kost geen extra boom-call: zelfde gecachte `/category` als het rooster, en beide tellen in één batch. De tekeningen zijn eigen werk in `currentColor`: de leverancier levert bij eindgroepen géén beeld, en foto's inkopen zou elf licenties vragen |
