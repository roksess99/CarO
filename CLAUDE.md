...
| Filters op eigenschap bij onderdelen: alleen motorolie | `lib/catalog/part-filters.ts`, @docs/DECISIONS.md | Als regel doen we het niet: leveranciersjargon als kop, en filteren verbergt artikelen waarvoor de fabrikant het veld niet invulde. **Motorolie is de gemeten uitzondering** — merk en inhoud hebben 100% dekking, viscositeit 99%, en het is precies wat er op de fles staat. Drie filters: merk, viscositeit (SAE), inhoud. Het filteren gebeurt in de shop en niet bij de leverancier, want viscositeit zit onder twee attribuut-ids waarvan er één niet in de facetten voorkomt — API-filteren zou die artikelen stil laten vallen. Een groep toevoegen? Eerst de dekking meten, dan pas het id in `FILTERABLE_GROUPS` |
